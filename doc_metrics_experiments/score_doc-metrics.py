...
def main(args):
    ...
    for sysname, cand in evs.sys_outputs.items():
        ...
        if args.doc:
            ...
            src = add_context(orig_txt=orig_src, context=ctxt_src, doc_ids=doc_ids, sep_token=sep_token)
            if not args.qe:
                ref = add_context(orig_txt=orig_ref, context=ctxt_ref, doc_ids=doc_ids, sep_token=sep_token)
            
            # add contexts to hypotheses text
            # if no reference is used, add candidate context
            if args.qe:
                cand = add_context(orig_txt=cand, context=cand, doc_ids=doc_ids, sep_token=sep_token)
            # if reference is used, add ref context
            else:
                cand = add_context(orig_txt=cand, context=ctxt_ref, doc_ids=doc_ids, sep_token=sep_token)
